...
/**
 * The event details page lists attendees who pledged to help but did not end up
 * contributing. Once an event is over, that list should no longer be shown, so a
 * no-show is not called out publicly after the fact (see issue #339).
 *
 * These tests render the details route for an event that is still active and for
 * one that has ended, and assert the attendees section is present only while the
 * event is active. The current user is granted event management rights so the
 * capability gate on the section is satisfied and the event state is the only
 * variable under test.
 */
class Event_Details_Attendees_Test extends Base_Test {
	/**
	 * @var Event_Factory
	 */
	...
	public function setUp(): void {
		...
		// Render as a logged-in user who is allowed to see the section.
